namespace OpenApiConnectedService.Package
{
    public static class Constants
    {
        public const string Author = "Dmitry Pavlov";
        public const string ExtensionCategory = "OpenAPI";
        public const string ExtensionName = "OpenAPI Connected Service";
        public const string ExtensionDescription = "Generates API client proxy code for OpenAPI (Swagger) web service with NSwag.";
        public const string ProviderId = "DmitryPavlov.OpenApiConnectedService";
        public const string Website = "https://github.com/dmitry-pavlov/openapi-connected-service/blob/master/README.MD";
        public const string Copyright = "Copyright © 2019";
    }
}